...
penumbra_broker_connected = Gauge("penumbra_broker_connected", "1 while a usable AMQP connection and channels are established, 0 otherwise; 0 with the process up means penumbra is not consuming")
penumbra_amqp_topology_rebuilds = Counter("penumbra_amqp_topology_rebuilds", "times the connection was torn down and rebuilt because a dead channel did not come back on its own; nonzero means aio-pika's own recovery failed")
